...
namespace DataAccess
{
    public class Util
    {
        public static DbSet MakeDBSetType<T>(DbContext DbContext) where T : class, new()
        {
        ...
        public static IList DataReaderToObjectList(Type generics, IDataReader reader, string fieldsToSkip = null, Dictionary<string, PropertyInfo> piList = null)
        {
            if (reader == null)
                return null;
            var items = (IList)MakeGenericType(typeof(List<>), generics);
            // Create lookup list of property info objects            
            if (piList == null)
            {
                piList = new Dictionary<string, PropertyInfo>();
                var props = generics.GetProperties(BindingFlags.Instance | BindingFlags.Public);
                foreach (var prop in props)
                    piList.Add(prop.Name.ToLower(), prop);
            }
            while (reader.Read())
            {
                var inst = Activator.CreateInstance(generics);
        ...
        {
            if (reader.IsClosed)
                throw new Exception("DataReader is closded");
